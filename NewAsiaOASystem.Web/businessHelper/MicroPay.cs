...
namespace NewAsiaOASystem.Web
{
    public class MicroPay
    {
      ...
      * @return 刷卡支付结果
      */
        public static string Run(string body, string total_fee, string auth_code,NewConfig config)
        {
            ...
            while (queryTimes-- > 0)
            {
                ...
                WxPayData queryResult = Query(out_trade_no, out succResult,config);
                //如果需要继续查询，则等待2s后继续
                if (succResult == 2)
                {
                    Thread.Sleep(2000);
	    ...
	    */
        public static WxPayData Query(string out_trade_no, out int succCode,NewConfig config)
        {
        ...
        */
        public static bool Cancel(string out_trade_no,NewConfig config, int depth = 0)
        {
            if (depth > 10)
            {
                return false;
