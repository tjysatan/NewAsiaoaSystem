...
namespace NewAsiaOASystem.Web
{
    public partial class test : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                string openid = Session["C_openid"].ToString();
                string total_fee = Session["total_fee"].ToString();
                //检测是否给当前页面传递了相关参数
                ...
                {
                    Log.Info("支付", "错误 : " + ex);
                    Response.Write("<span style='color:#FF0000;font-size:20px'>" + "下单失败11，请返回重试" + "</span>");
                    submit.Visible = false;
                }
            }
        }
